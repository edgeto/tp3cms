...
class BaseController extends Controller {
    ...
    protected function _initialize(){
        ...
        $this->assign('loginUrl',C("DEFAULT_URL.USER").'/public/login?referer_url='.C("DEFAULT_URL.V").__SELF__);
        //配置
        $this->config();
        //导航
        $this->nav();
    ...
    public function config(){
        ...
        //标题、关键词和描述
        $this->assign('title',C('video_title').'-'.C('main_title'));
        $this->assign('keywords',C('video_keywords'));
        $this->assign('description',C('video_desc'));
    ...
    public function nav(){
        ...
        //视频头部
        $nav = $NavPosition->getNavByPosition(6);
        // 面包屑
        $breadcrumbs[]  = array(
            'url' => '/',
            'name' => '首页'
        ...
        if($id){
            $id = $id[1];
            $category_name = D('Video')->getCategoryByVideoId($id);
            if($category_name){
