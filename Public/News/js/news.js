...
$(".slideBox").slide({mainCell:".bd ul",autoPlay:true,effect:'left'});
// 收藏
/*$('.collect .glyphicon-heart').click(function(){
	if($(this).hasClass('in')){
		// 弹窗
	}else{
...
article.config(function($interpolateProvider) {
    $interpolateProvider.startSymbol('<{');
    $interpolateProvider.endSymbol('}>');
});
article.controller('ArticleCate',['$scope', '$http',function($scope,$http) {
	...
	var id = angular.element(document.querySelector('#article_id')).val();
 	$scope.getCommentList = function(e){
 		$http({
			  method: 'GET',
			  url: '/article/getComment/id/'+id+'/p/'+$scope.p
			}).then(function successCallback(response) {
			    // this callback will be called asynchronously
			    // when the response is available
 	...
 	$scope.doComment = function(e){
 		if($scope.isLogin()){
 			...
 			if(commentContent){
 				$http({
				  method: 'POST',
				  // url: '/article/doComment/id/'+id+'content/'+commentContent,
				  url: '/article/doComment',
